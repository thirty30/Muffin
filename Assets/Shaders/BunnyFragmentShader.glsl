...
void main()  
{
	...
	{
		vec3 texture0 = texture( un_Samplers[0], in_fUVx2.st ).rgb;
		if (texture0.r < 0.2f)
		{
			discard;
		}
		vec3 texture1 = texture( un_Samplers[1], in_fUVx2.st ).rgb;
		vVertexMaterialColour = texture0 * 1.0f + texture1 * 0.0f;
	}
